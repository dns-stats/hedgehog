        ...
        if(small==1){
          p <- gvisBarChart(df, xvar='x', yvar='y',
                            options=list(legend="none", title=title, height=600, width=940,
                                         ...
                                         chartArea="{left:100,top:50,width:\"88%\",height:\"80%\"}"))
        } else {
          p <- gvisBarChart(df, xvar='x', yvar='y',
                            options=list(legend="none", title=title, height=600, width=940,
...
generateYaml <- function(dsccon) {
  ...
  # Now process the data from the data frame into yaml format
    yaml_out <- list('version'=3, service=server_service, 'start-period'=paste(GET$start, ":00Z", sep=""), metric=metric)
...
generatePlotFile <- function(plttitle, pltnm, ddpltid, plot_file, simple_start, simple_stop, svrid, pltid, gvis, ndarr, dsccon) {
  ...
  else if  (pltnm %in% format3_bgpprefix)         {xlab <- "Subnet (As seen in BGP)"}
  else if  (pltnm %in% format3_asn)               {xlab <- "Subnet ASN"}
  ...
  if      (pltnm %in% rssac)                  {ylab <- "Queries/min"}
           ...
           pltnm %in% format3_bgpprefix ||
           pltnm %in% format3_asn)                {ylab <- "Average Query Rate (q/sec)"}
  else if (pltnm == 'traffic_sizes_small' ||
  ...
  if      (is.null(df))                               {plot_file <- "plots/no_connection.png"}
           ...
           pltnm %in% format3_bgpprefix ||
           pltnm %in% format3_asn)                        {barPlot             (df, plot_file, mytitle, xlab, ylab, gvis)}
           ...
           pltnm == 'client_subnet2_accum' ||
           pltnm == 'client_subnet_vs_tld')           {stackedBarPlot(df, plot_file, mytitle, "Subnet (IPv4/8 or IPv6/32)", "Average Query Rate (q/sec)", gvis, pltnm)}
