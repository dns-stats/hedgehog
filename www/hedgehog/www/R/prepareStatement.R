    ...
    if(class(dsccon) != "try-error"){
      ...
      skipped_sql="NOT IN ('-:SKIPPED_SUM:-', '-:SKIPPED:-')"
        ...
        switch(statementNm,
               ...
               by_region                            = { rs <- try(dbSendQuery(dsccon, "PREPARE by_region                            (INTEGER, INTEGER, TIMESTAMPTZ, TIMESTAMPTZ, TEXT) AS           select to_timestamp((extract(epoch from sq.sx)::int / (extract(epoch from $4::timestamp - $3::timestamp + interval '1 minute')/1440)::int )*(extract(epoch from $4::timestamp - $3::timestamp + interval '1 minute')/1440)::int) AS x, sq.skey as key, avg(sq.sy) as y from (SELECT d.starttime AS sx, CASE WHEN (n.region   = '' OR n.region   IS NULL) THEN 'Other'       ELSE n.region   END AS skey, sum(d.value)/60.0 AS sy FROM dsc.data d WHERE d.server_id=$1 AND d.plot_id=$2 AND d.starttime>=$3 AND d.starttime<=$4 AND d.node_id = ANY (string_to_array($5, ',')::integer[]) GROUP BY sx, skey) as sq GROUP BY x, key;"))},
               by_region_all_nodes                  = { rs <- try(dbSendQuery(dsccon, "PREPARE by_region_all_nodes                  (INTEGER, INTEGER, TIMESTAMPTZ, TIMESTAMPTZ) AS                 select to_timestamp((extract(epoch from sq.sx)::int / (extract(epoch from $4::timestamp - $3::timestamp + interval '1 minute')/1440)::int )*(extract(epoch from $4::timestamp - $3::timestamp + interval '1 minute')/1440)::int) AS x, sq.skey as key, avg(sq.sy) as y from (SELECT d.starttime AS sx, CASE WHEN (n.region   = '' OR n.region   IS NULL) THEN 'Other'       ELSE n.region   END AS skey, sum(d.value)/60.0 AS sy FROM dsc.data d WHERE d.server_id=$1 AND d.plot_id=$2 AND d.starttime>=$3 AND d.starttime<=$4 GROUP BY sx, skey) as sq GROUP BY x, key;"))},
               by_instance                          = { rs <- try(dbSendQuery(dsccon, "PREPARE by_instance                          (INTEGER, INTEGER, TIMESTAMPTZ, TIMESTAMPTZ, TEXT) AS           select to_timestamp((extract(epoch from sq.sx)::int / (extract(epoch from $4::timestamp - $3::timestamp + interval '1 minute')/1440)::int )*(extract(epoch from $4::timestamp - $3::timestamp + interval '1 minute')/1440)::int) AS x, sq.skey as key, avg(sq.sy) as y from (SELECT d.starttime AS sx, CASE WHEN (n.subgroup = '' OR n.subgroup IS NULL) THEN 'No Instance' ELSE n.subgroup END AS skey, sum(d.value)/60.0 AS sy FROM dsc.data d WHERE d.server_id=$1 AND d.plot_id=$2 AND d.starttime>=$3 AND d.starttime<=$4 AND d.node_id = ANY (string_to_array($5, ',')::integer[]) GROUP BY sx, skey) as sq GROUP BY x, key;"))},
               by_instance_all_nodes                = { rs <- try(dbSendQuery(dsccon, "PREPARE by_instance_all_nodes                (INTEGER, INTEGER, TIMESTAMPTZ, TIMESTAMPTZ) AS                 select to_timestamp((extract(epoch from sq.sx)::int / (extract(epoch from $4::timestamp - $3::timestamp + interval '1 minute')/1440)::int )*(extract(epoch from $4::timestamp - $3::timestamp + interval '1 minute')/1440)::int) AS x, sq.skey as key, avg(sq.sy) as y from (SELECT d.starttime AS sx, CASE WHEN (n.subgroup = '' OR n.subgroup IS NULL) THEN 'No Instance' ELSE n.subgroup END AS skey, sum(d.value)/60.0 AS sy FROM dsc.data d WHERE d.server_id=$1 AND d.plot_id=$2 AND d.starttime>=$3 AND d.starttime<=$4 GROUP BY sx, skey) as sq GROUP BY x, key;"))},
               by_city                              = { rs <- try(dbSendQuery(dsccon, "PREPARE by_city                              (INTEGER, INTEGER, TIMESTAMPTZ, TIMESTAMPTZ, TEXT) AS           select to_timestamp((extract(epoch from sq.sx)::int / (extract(epoch from $4::timestamp - $3::timestamp + interval '1 minute')/1440)::int )*(extract(epoch from $4::timestamp - $3::timestamp + interval '1 minute')/1440)::int) AS x, sq.skey as key, avg(sq.sy) as y from (SELECT d.starttime AS sx, CASE WHEN (n.city     = '' OR n.city     IS NULL) THEN 'No City'     ELSE n.city     END AS skey, sum(d.value)/60.0 AS sy FROM dsc.data d WHERE d.server_id=$1 AND d.plot_id=$2 AND d.starttime>=$3 AND d.starttime<=$4 AND d.node_id = ANY (string_to_array($5, ',')::integer[]) GROUP BY sx, skey) as sq GROUP BY x, key;"))},
               by_city_all_nodes                    = { rs <- try(dbSendQuery(dsccon, "PREPARE by_city_all_nodes                    (INTEGER, INTEGER, TIMESTAMPTZ, TIMESTAMPTZ) AS                 select to_timestamp((extract(epoch from sq.sx)::int / (extract(epoch from $4::timestamp - $3::timestamp + interval '1 minute')/1440)::int )*(extract(epoch from $4::timestamp - $3::timestamp + interval '1 minute')/1440)::int) AS x, sq.skey as key, avg(sq.sy) as y from (SELECT d.starttime AS sx, CASE WHEN (n.city     = '' OR n.city     IS NULL) THEN 'No City'     ELSE n.city     END AS skey, sum(d.value)/60.0 AS sy FROM dsc.data d WHERE d.server_id=$1 AND d.plot_id=$2 AND d.starttime>=$3 AND d.starttime<=$4 GROUP BY sx, skey) as sq GROUP BY x, key;"))},
               by_country                           = { rs <- try(dbSendQuery(dsccon, "PREPARE by_country                           (INTEGER, INTEGER, TIMESTAMPTZ, TIMESTAMPTZ, TEXT) AS           select to_timestamp((extract(epoch from sq.sx)::int / (extract(epoch from $4::timestamp - $3::timestamp + interval '1 minute')/1440)::int )*(extract(epoch from $4::timestamp - $3::timestamp + interval '1 minute')/1440)::int) AS x, sq.skey as key, avg(sq.sy) as y from (SELECT d.starttime AS sx, CASE WHEN (n.country  = '' OR n.country  IS NULL) THEN 'No Country'  ELSE n.country  END AS skey, sum(d.value)/60.0 AS sy FROM dsc.data d WHERE d.server_id=$1 AND d.plot_id=$2 AND d.starttime>=$3 AND d.starttime<=$4 AND d.node_id = ANY (string_to_array($5, ',')::integer[]) GROUP BY sx, skey) as sq GROUP BY x, key;"))},
               by_country_all_nodes                 = { rs <- try(dbSendQuery(dsccon, "PREPARE by_country_all_nodes                 (INTEGER, INTEGER, TIMESTAMPTZ, TIMESTAMPTZ) AS                 select to_timestamp((extract(epoch from sq.sx)::int / (extract(epoch from $4::timestamp - $3::timestamp + interval '1 minute')/1440)::int )*(extract(epoch from $4::timestamp - $3::timestamp + interval '1 minute')/1440)::int) AS x, sq.skey as key, avg(sq.sy) as y from (SELECT d.starttime AS sx, CASE WHEN (n.country  = '' OR n.country  IS NULL) THEN 'No Country'  ELSE n.country  END AS skey, sum(d.value)/60.0 AS sy FROM dsc.data d WHERE d.server_id=$1 AND d.plot_id=$2 AND d.starttime>=$3 AND d.starttime<=$4 GROUP BY sx, skey) as sq GROUP BY x, key;"))},
               ...
               load_time                            = { rs <- try(dbSendQuery(dsccon, "PREPARE load_time                        (INTEGER, INTEGER, TIMESTAMPTZ, TIMESTAMPTZ, TEXT) AS               SELECT d.starttime AS x, n.name AS key, d.value AS y, d.key2 as serial FROM dsc.data d WHERE d.server_id=$1 AND d.plot_id=$2 AND d.starttime>=$3 AND d.starttime<=$4 AND d.node_id = ANY (string_to_array($5, ',')::integer[]) order by serial;"))},
               load_time_all_nodes                  = { rs <- try(dbSendQuery(dsccon, "PREPARE load_time_all_nodes              (INTEGER, INTEGER, TIMESTAMPTZ, TIMESTAMPTZ) AS                     SELECT d.starttime AS x, n.name AS key, d.value AS y, d.key2 as serial FROM dsc.data d WHERE d.server_id=$1 AND d.plot_id=$2 AND d.starttime>=$3 AND d.starttime<=$4 order by serial;"))},
               zone_size                            = { rs <- try(dbSendQuery(dsccon, "PREPARE zone_size                        (INTEGER, INTEGER, TIMESTAMPTZ, TIMESTAMPTZ, TEXT) AS               SELECT d.starttime, avg(d.value) AS y, d.key2 AS x FROM dsc.data d WHERE d.server_id=$1 AND d.plot_id=$2 AND d.starttime>=$3 AND d.starttime<=$4 AND d.node_id = ANY (string_to_array($5, ',')::integer[]) group by d.starttime, d.key2 order by x;"))},
               zone_size_all_nodes                  = { rs <- try(dbSendQuery(dsccon, "PREPARE zone_size_all_nodes              (INTEGER, INTEGER, TIMESTAMPTZ, TIMESTAMPTZ) AS                     SELECT d.starttime, avg(d.value) AS y, d.key2 AS x FROM dsc.data d WHERE d.server_id=$1 AND d.plot_id=$2 AND d.starttime>=$3 AND d.starttime<=$4 group by d.starttime, d.key2 order by x;"))},
