...
/* Both are bumped from 2.0 version by 1 since DDL 10 is not run on fresh install.
   This is because it requires super user priviledges so is done
   as part of database_create. */
...
INSERT INTO visible_plot (name, display_category, display_name, title) VALUES ('traffic_volume_difference', '9.RSSAC', 'Traffic volume difference', 'The differences between queries and responses by transport and IP version');
...
INSERT INTO dataset_visible_plot (visible_plot_id, dataset_id) VALUES ((SELECT id FROM visible_plot WHERE name='traffic_volume_difference'), (SELECT id FROM dataset WHERE name='traffic_volume_queries'));
INSERT INTO dataset_visible_plot (visible_plot_id, dataset_id) VALUES ((SELECT id FROM visible_plot WHERE name='traffic_volume_difference'), (SELECT id FROM dataset WHERE name='traffic_volume_responses'));
